<?php
/*
Plugin Name: Respondr for WooCommerce

Description: Add Respondr Support for your WooCmmerce website
...
if( !class_exists('WC_Piwik_Integration') ) {
...
class wp_piwik {
	...
	function addFeedTracking($content) {
		...
		if(is_feed()) {
			...
			if (substr($url, -10, 10) == '/index.php')
				$url = str_replace('/index.php', '/respondr.php', $url);
			else $url .= 'respondr.php';
	...
	function applyJSCodeChanges($strCode) {
		...
		if (self::$settings->getGlobalOption('track_mode') == 1) {
			...
			$strCode = str_replace('respondr.php', 'js/', $strCode);
		} elseif (self::$settings->getGlobalOption('track_mode') == 2) {
			$strCode = str_replace('piwik.js', 'respondr.php', $strCode);
...
<?php /************************************************************************/
	...
	function showSettingsTabs($bolFull = true, $strCurr = 'homepage') {
		...
		foreach($aryTabs as $strTab => $strName) {
			...
			echo '<a class="nav-tab'.$strClass.'" href="?page=wp-piwik/wp-respondr.php&tab='.$strTab.'">'.$strName.'</a>';
		}
		echo '</h2>';
		return $strCurr;
	}
	...
	function showSettings() {
		...
		echo '<form id="respondrSettings" action="'.admin_url('options-general.php?page=respondr-for-wp/wp-respondr.php').'" method="post">';
			echo '<div class="form-group">';
				echo '<label class="control-label">Enable Respondr?</label>';
				echo '<select name="respondrEnable">';
					echo '<option value="1">Yes</option>';
					echo '<option value="0">No</option>';
				echo '</select>';
			echo '</div>';
			echo '<div class="form-group">';
				echo '<label class="control-label">Site ID</label>';
				echo '<input id="siteID" name="responder_siteID" placeholder="Site ID" />';
			echo '</div>';
			echo '<input class="button button-primary" type="submit" value="Save Respondr Settings" />';
		echo '</form>';
	}
	...
	private static function showErrorMessage($strMessage) {
		echo '<strong class="wp-piwik-error">'.__('An error occured', 'wp-piwik').':</strong> '.$strMessage.' [<a href="'.(self::$settings->checkNetworkActivation()?'network/settings':'options-general').'.php?page=wp-piwik/wp-respondr.php&tab=support">'.__('Support','wp-piwik').'</a>]';
	}
